...
const QUERY = `*[_type == "edital" && ativo == true] | order(status asc) {
  _id, titulo, orgao, objeto, periodo, status, link
}`

export default async function EditaisPage() {
  const sanityData: Edital[] = await sanityClient.fetch(QUERY)
  const editais = sanityData.length > 0 ? sanityData : fallback
    ...
    <main>
      ...
      <div className="max-w-4xl mx-auto px-4 py-14">
        <p className="text-gray-600 text-base mb-8 max-w-2xl">
          A ASESP atua em parceria com órgãos públicos e privados para ampliar o acesso da comunidade surda à cultura, educação e oportunidades.
        </p>

        <div className="flex flex-col gap-5">
          {editais.map((edital) => {
            const cfg = statusConfig[edital.status] ?? statusConfig['concluido']
